...
namespace Config {
    ...
    // These are calculated from the screen and tile sizes.
    constexpr int GRID_COLS = SCREEN_WIDTH_PX / TILE_SIZE_PX; // 480 / 16 = 30
    constexpr int GRID_ROWS = SCREEN_HEIGHT_PX / TILE_SIZE_PX; // 272 / 16 = 17


    // --- Game Mechanics ---
    // The speed of the game, representing the delay between ticks in milliseconds.
    // A lower value makes the game faster.
    constexpr int GAME_TICK_SPEED_MS = 150;


    // --- Visuals & Colors ---
    // Define all game colors here for easy theming.
    const QColor COLOR_BACKGROUND = Qt::black;
    const QColor COLOR_SNAKE_BODY = Qt::green;
    const QColor COLOR_FOOD = Qt::red;
